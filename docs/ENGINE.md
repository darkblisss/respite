...
- Class: `pickClass(state, { id }, env)` (health refills; at camp the camp's note `hp` too; emits `class:picked {id}`).
...
Predictable commands: `startSkill, stopSkill, startHunt, pullBack, setHide, pickClass, equip, unequip, unequipTool, moveItem, sellItem, salvage, useChest, repair, reorder, buyRemedy, buySmuggler, travel, claimBounty, hireAgent, deployAgent, buyCompanion, setCompanion`.
