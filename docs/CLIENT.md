...
| `src/client/ui/monster-art.js` | every foe's own drawing, `MONSTER_ART` keyed by monster id, and v4's five `KIND_ART` drawings as the fallback; `monsterArt()` in popups/foe.js draws from it (classes in pages.css) | hunt, character, popups |
...
| `src/client/ui/hunt-ring.js` | the Closing Ring, the Hunt page's fight on one canvas: `huntRing({ onFoe, onMode })` -> `{ node, sync(snapshot), blow(hunt:fx event), mode, clear(), destroy() }`. Each foe stands on the ring at the distance its next swing is away; the page hands it a snapshot every store frame and every blow it hears of, and it runs the timers on at the screen's rate between them. Wide (a card of 820px and up) is 1130 by 500 units with a plate beside every foe; narrow is the card's own pixels, discs only, and the page lists the foes under it (`onMode` says which) | hunt |
| `src/client/ui/recap.js` | the walk's word on the last encounter: `recapTracker()` fed the hunt every frame, `felled(key)` and `broke()` from their events | hunt |
...
- `store:rejected` the error (warn); `store:news` mail: "The post: 200g" or "The post: 12 Slag Ore" (gold); away: "Welcome back: away 3h 12m" (info); `party:spoils` "Your share: 4 kills, 1,210 XP, 96g, 3 drops" (gold, party icon, once a minute at most), "Sovereign felled: Veiled Essence" (gold, crown) when the party brought one down, and, on a fall, "You fell beside your party" (bad). `chronicle.js` writes no camp line for a share, only for a Sovereign's fall and a death, so these toasts are most of the word the player gets.
...
- **The stage**: the Hunt card's fight and walk share one stage (`.hunt-stage`, the card carrying `data-phase` quiet, search or fight, `data-vast` and `data-size` wide or narrow). The walk: your face, your discipline's glyph before your name and your health (no Veil: every encounter opens it the same way), the way on with its countdown, the last encounter's tally, and What waits (the zone's three kinds by how often they come, how many at once, the Elite odds with the Sovereign's under them; once one has found you, the Sovereign itself). The fight is the ring, with the encounter and its window over it: "Another steps out in", or once the zone's `joins` have all come "The last of them is out", and in a Sovereign's fight the fight's own clock ("Fighting for"). A Sovereign is last on the roster behind its two Elites, so the ring strikes a guard first; it still takes the head of the ring, stepping out of the dark where the walk's vast eyes were, with a pale ring going out when it lands. A dodged blow is a quick step aside and back. Under the stage, the run's numbers and what it has turned up, a tile a thing; under the Zones, the Quarry as one plate with a tab for each of the region's four.
- **Nothing heals on its own** (engine 9): the quiet walk of a hunter home under half health says so and points at the Satchel; a remedy's item popup has **Drink** (the `useRemedy` command: at camp only, not while a party's fight holds your health, not a bottle too weak for your ground), saying how much it will put back. The Standing card and the profile sheet (`sheetFrom(stats, klass, tier)`) show Block, Dodge, Lifesteal and Veil Power, and Defence with the share it stops on your ground (`mitigation`); the item popup and the bench table show a piece's Block, Dodge and Veil Power. The Quarry plate shows a foe's Defence as the share of your blow it stops, what a kill pays after hunting beneath yourself (`overLevel`), and says when you stand past the ground; so does the zone popup ("Beneath you").
- **The walk's tally**: between encounters of your own hunt, the walk says what the last one came to (kills, XP, what it left, how long it took; a Sovereign's adds felled or broke away and its Essence). The hunt keeps running totals and nothing per encounter, so `ui/recap.js` holds the totals of the last walk the page saw and reads the difference on the walk after the fight. An encounter whose walk in was not seen (another page, a hidden tab caught up in one go) gets no tally rather than a wrong one.
- **The Hunt page** shows the shared fight on the same stage when `store.partyHunt` names this player. Its blows come from no local simulation (a shared fight has none), so the ring reads them off what each answer took: a foe that lost health flashes, a hunter who did is clawed. It runs each foe's swing timer on from the answer (`enc.foes[].timer`), and a line from every foe to whoever its next blow is for burns brighter as the blow comes round; a party's foes keep to the hunter they picked until that hunter falls, and the plate says so ("Strikes Thane in 1.2s"; on a phone, the list's "On you", "On Thane"). The walk shows the party as rows, you first: face, glyph, name, health and share of the take. KPIs become Encounters, Your damage, Party damage, Your share (the split a kill actually pays by) and Time out. The walk to the ground's Sovereign says so as your own hunt's does ("Something vast approaches · Here in 3s"), and its fight gives its own clock in place of the window. A member who joined after an encounter had begun is in the session but not in that fight; the fight's head says "In on the next encounter" and their row reads "Waiting".
