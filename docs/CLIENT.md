...
- **The walk's tally**: between encounters of your own hunt, the empty arena says what the last one came to (kills, XP, what it left, how long it took; a Sovereign's adds felled or broke away and its Essence). The hunt keeps running totals and nothing per encounter, so `ui/recap.js` holds the totals of the last walk the page saw and reads the difference on the walk after the fight. An encounter whose walk in was not seen (another page, a hidden tab caught up in one go) gets no tally rather than a wrong one.
- **The Hunt page** shows the shared fight in the same `.arena` when `store.partyHunt` names this player. It runs no float or shake layer: those are drawn from `hunt:fx` events raised by the local simulation, and a shared fight has none, so the arena is quieter by design. KPIs become Encounters, Your damage, Party damage, Your share (the split a kill actually pays by) and Time out. Each foe's card says whom it is going for ("On you", "On Thane"): a party's foes keep to the hunter they picked until that hunter falls. The walk to the ground's Sovereign says so as your own hunt's does ("Something vast approaches · Here in 3s"), and its fight gives its anger in place of the encounter's number. A member who joined after an encounter had begun is in the session but not in that fight; the arena says "Waiting · In on the next encounter" and their band row reads "Waiting".
