...
- The shelf (migration 010). Gear cannot pool -- one Slag Sword is Rare and worked to +7, the next is a plain Common -- so it is grouped by base instead: the page shows the sword once, at the cheapest it can be had for, and opening it lists the pieces. Both carry no `seller_id`, `seller_name` or `buyer_id`, and a caller's own lots **are** included (unlike a material pool), because gear is bought a piece at a time and a seller should still see the rest of the shelf.
  - `market_rarity_rank(p_rarity text) returns int`: the rarity ladder as a number, matching `GameData.RARITIES`. 0 for a rarity the database has never heard of, which passes no floor.
  - `market_bases(p_q text default '', p_kind text default null, p_tier int default null, p_rarity text default null, p_sort text default 'price', p_limit int default 50) returns table(item_base, item_name, item_kind, item_tier, lots int, mine_lots int, qty_left bigint, price_min bigint, price_max bigint, newest timestamptz, rarities jsonb)`: open, unexpired listings that are **not** materials, one row an `item_base`, `price` or `newest` order, at most 100. `rarities` is `[{ rarity, lots, qty, min }]`, least rarity first. `item_name` is the plainest name on the shelf (lowest rarity, then shortest) and a fallback only: a client knows the base and names it itself. `p_rarity` is a **floor** -- `'rare'` means Rare and up -- and it decides `price_min` too, so the price shown is the cheapest that passes the filter. `p_kind = 'material'` returns nothing.
  - `market_base_listings(p_base text, p_rarity text default null, p_limit int default 50) returns table(...)`: the same columns `market_browse()` answers with, for one base, cheapest first then oldest. The same rarity floor. An empty `p_base` returns nothing.
...
`party` is `sessionView()` of the party hunt the caller is out on, and absent when they are not. Beside who is in it and their shares, it says `vast: true` on the walk to the ground's Sovereign; its `enc` gives each foe's `target` (the user id it is going for) and `timer` (ms until it swings), `reinforceIn` (ms until the window brings another; null in a Sovereign's fight, which nothing else joins). A Sovereign no longer angers on a clock, so there is no `enrage` or `enrageIn` (engine 9); its guard is first on the roster and it is last. Every hunter, in the session and in the encounter, carries `klass` (their discipline or null); the encounter's add `veil` (0 to 100) and `volley` (a Mage's opening casts left).
