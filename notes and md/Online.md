...
- **Guests** can play straight away, but a guest camp lives only in that browser tab and is gone when it closes. The Market, Party and Leaderboard ask guests to sign in.
...
- **Hunting together:**
  - While you hunt, each other member hunting the same tier and zone at the same moment adds +5% Hunt XP, up to +15%. The bonus is small on purpose: the draw is the company, not the multiplier.
...
### Leaderboard and presence

- **Leaderboard:** the top 50, board by board: Total, Hunt by discipline, each gathering trade, each artisan bench, Wealth (the value a player has made themselves, never what they traded for) and Monsters killed.
