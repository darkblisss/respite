...
-- 386 keys moved with it.
--
-- A save heals itself, because the client migrates it on load and writes it
-- back. These three tables never pass through that, so they are rewritten here:
--
...
--
-- item_name is a stored string the market reads back as-is, so it is rewritten
-- too. A gear listing's name wraps the base in its prefix and plus
-- ("Wounding Gloam Sword +7"), which is why the old name is replaced inside it
-- rather than the whole column being overwritten.
--
-- Safe to run twice: a second pass finds no old keys left to move.
-- ============================================================

begin;

create temporary table key_moves (
...
insert into key_moves (old_key, new_key, old_name, new_name) values
  ('bog_delve', 'mire_delve', 'Mire Ore', 'Mire Ore'),
...
-- item_base can be wrong on its own if a row was ever written without the key.
update public.market_listings l
   set item_base = m.new_key
  from key_moves m
