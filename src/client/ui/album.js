/* ============================================================
   Respite · ui/album.js · What They Have Seen
   ------------------------------------------------------------
   The Collection as the profile card draws it (ui/profile.js):
   the ledger of things and the bestiary, off the same roll map
   ui/collection.js reads -- a stranger's, as the realm publishes
   it, or your own save when the name is yours. The Character
   page keeps its own panel in ui/collection.js.
 ...
 */
export function albumPanel({ onEntry = null } = {}) {
  let mode = MODES[0].id;
  let kind = KINDS[0].id;
  let group = 0;
  let rolls = {};
  ...
  const total = h("span.alb-total");
  const totalBar = h("i");
  const sub = h("p.alb-sub");
  const album = h("div.alb-album", { role: "list" });
  const openHead = h("div.alb-open-head");
  const grid = h("div.alb-tiles");
  const node = h("section.alb",
    h("div.alb-head",
      h("h2.alb-title", "Collection"),
      h("div.alb-sum", total, h("div.bar.bar-gold.bar-thin", totalBar))),
    sub, album, h("div.alb-open", openHead, grid));

  const modeRow = h("div.seg.seg-full.alb-modes", { role: "tablist", "aria-label": "Collection" });
  const kindList = h("div.alb-kinds", { role: "tablist", "aria-label": "Kind" });
  const foeNote = h("div.alb-foe-note");
  const filters = h("div.alb-filters", modeRow, kindList, foeNote);
  ...
  function drawFilters(itemSeen, foeSeen) {
    ...
    if (mode === "items") {
      kindList.replaceChildren(...KINDS.map((k) => {
        ...
        return h("button.alb-kind", {
          type: "button", role: "tab", "aria-selected": k.id === kind ? "true" : "false", dataset: { kind: k.id },
        },
          h("span.alb-kind-top", h("span.alb-kind-name", k.name), h("span.alb-kind-n", `${fmtWhole(seen)}/${fmtWhole(k.count)}`)),
      ...
      foeNote.replaceChildren(
        h("div.alb-kind-top", h("span.alb-kind-name", `${fmtWhole(foeSeen)} of ${fmtWhole(BESTIARY_COUNT)} foes seen`)),
        h("p.alb-foe-copy", "Sovereigns show in gold. Pick a ground to see who you have put down there."),
  ...
  function drawAlbum(gs) {
    ...
    album.replaceChildren(...gs.map((g, i) => {
      ...
      return h("button.alb-row", {
        ...
        h("span.alb-row-name", { class: !seen && "is-none" }, g.name),
        h("span.alb-sqs", { "aria-hidden": "true", style: { "--n": String(g.items.length) } },
          g.items.map((d) => h("span.alb-sq", { class: isSeen(d) && "is-seen", "data-sov": foes && d.archetype === "sovereign" ? "" : null },
            foes ? iconEl(foeGlyph(d)) : null))),
        h("span.alb-row-n", `${fmtWhole(seen)}/${fmtWhole(g.items.length)}`));
    }));
  }
  ...
  function drawOpen(gs) {
    ...
    openHead.replaceChildren(h("h3.alb-open-title", g ? g.name : ""),
      h("span.alb-open-n", `${fmtWhole(seen)} of ${fmtWhole(items.length)} ${mode === "foes" ? "seen" : "found"}`));
  ...
  function itemTile(def) {
    if (!found(rolls, def.id)) {
      return h("div.alb-tile.is-locked", { title: "Not found yet" }, h("span.alb-tile-art", { "aria-hidden": "true" }, iconEl(def.icon || "unknown")));
    }
    const art = hasArt(def) ? artEl(def, { variant: "cut" }) : iconEl(def.icon || "unknown");
    return h("button.alb-tile", { type: "button", title: def.name, "aria-label": def.name, dataset: { item: def.id } },
      h("span.alb-tile-art", { class: { "art-paint": hasArt(def) }, "aria-hidden": "true" }, art));
  }
  ...
  function foeCard(mob) {
    ...
    if (!kills) {
      return h("div.alb-foe.is-locked", { class: sov && "is-sovereign" },
        h("span.alb-foe-art", { "aria-hidden": "true" }, iconEl(foeGlyph(mob))),
        h("span.alb-foe-name", "Unseen"));
    }
    return h("button.alb-foe", { type: "button", class: sov && "is-sovereign", dataset: { monster: mob.id } },
      h("span.alb-foe-art", { "aria-hidden": "true", html: monsterArt(mob) }),
      h("span.alb-foe-name", mob.name),
      h("span.alb-foe-n", `${fmtWhole(kills)} slain`));
  }
